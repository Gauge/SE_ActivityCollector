...
namespace ActivityCollectorPlugin.Descriptions
{
    public class BlockDefinitionDescription : ISQLQueryData
    {
        ...
        public string GetQuery()
        {
            ...
            final.Append($@"IF NOT EXISTS (SELECT * FROM definition_blocks WHERE [type_id] = '{TypeId}' AND [subtype_id] = '{SubTypeId}' AND name = '{Name}' AND cube_size = '{CubeSize}' AND max_integrity = {MaxIntegrity} AND critical_integrity_ratio = {CriticalIntegrityRatio} AND general_damage_multiplier = {GeneralDamageMultiplier} AND disassemble_ratio = {DisassembleRatio} AND deformation_ratio = {DeformationRatio} AND mass = {Mass} AND is_air_tight = '{IsAirTight}' AND size_x = {SizeX} AND size_y = {SizeY} AND size_z = {SizeZ} AND model_offset_x = {ModelOffestX} AND model_offset_y = {ModelOffestY} AND model_offset_z = {ModelOffestZ} AND CONVERT(VARCHAR, description) = '{Description}')
BEGIN
INSERT INTO definition_blocks ([session_id], [type_id], [subtype_id], [cube_size], [name], [max_integrity], [critical_integrity_ratio], [general_damage_multiplier], [disassemble_ratio], [deformation_ratio], [mass], [is_air_tight], [size_x], [size_y], [size_z], [model_offset_x], [model_offset_y], [model_offset_z], [description], [timestamp])
VALUES ({ActivityCollectorPlugin.CurrentSession}, '{TypeId}', '{SubTypeId}', '{CubeSize}', '{Name}', {MaxIntegrity}, {CriticalIntegrityRatio}, {GeneralDamageMultiplier}, {DisassembleRatio}, {DeformationRatio}, {Mass}, '{IsAirTight}', {SizeX}, {SizeY}, {SizeZ}, {ModelOffestX}, {ModelOffestY}, {ModelOffestZ}, '{Description}', '{Helper.DateTimeFormated}')
");
            ...
            foreach (var component in Components)
            {
                ...
                values.AppendLine($"({ActivityCollectorPlugin.CurrentSession}, @block_id, (SELECT TOP 1 id FROM definition_components WHERE [type_id] = '{component.Definition.Id.TypeId.ToString()}' AND [subtype_id] = '{component.Definition.Id.SubtypeId.ToString()}' AND [session_id] = {ActivityCollectorPlugin.CurrentSession}), {component.Count}, {index}, '{Helper.DateTimeFormated}'){(index < Components.Length - 1 ? "," : "")}");
                index++;
            }
